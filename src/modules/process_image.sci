...
function [original_image, noisy_image] = generate_test_image(parameters)
    ...
    // parameters.noise_level_factor est entre 0 et 1 (e.g., 0.05 pour 5%)
    noisy_image = double(blurred_image);
    noise_mask = rand(size(blurred_image)) < parameters.noise_level_factor;
...
function output_image = apply_median_filter(input_image, size)
